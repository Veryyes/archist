...
def generate_arches():
    ...
    for name in arch_names:
        ...
        a["name"] = name
        a["ks"] = (
            f"keystone.KS_ARCH_{name}" if hasattr(keystone, f"KS_ARCH_{name}") else "-1"
        )
        a["cs"] = (
            f"capstone.CS_ARCH_{name}" if hasattr(capstone, f"CS_ARCH_{name}") else "-1"
        )
        a["uc"] = (
            f"unicorn.UC_ARCH_{name}" if hasattr(unicorn, f"UC_ARCH_{name}") else "-1"
        )
        a["ql"] = (
            f"qiling.const.QL_ARCH.{name}"
